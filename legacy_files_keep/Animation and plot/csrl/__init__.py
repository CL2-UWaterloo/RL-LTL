...
class ControlSynthesis:
    ...
    def MC_learning(self, model, LTL_formula, predicates, n_steps=150, C=3, tow=1, n_samples=300,
                    search_depth=None, N={}, W={}, Q={}, P={}, verbose=0,
        ...
        for k in range(K):
            ...
            if outcome[0]:
                ...
                if verbose>0: print("success ep:",k+1,"/",K)
    ...
    def MCTS_rec(self, model, root, LTL_formula, trajectory, predicates, N={}, W={}, Q={}, P={}, visited=set(), C=1, depth=100, random_move_chance=0, foo=0):
        ...
        non_None_idx = self.transition_probs[root]!=None
        try:
            # print(root)
            next_move = (U + Q[root])[non_None_idx].argmax()
        except:
            print(U)
            print(root)
            print(W)
    ...
    def MCTS(self, model, root, LTL_formula, predicates, trajectory, N, W, Q, P, visited=set(), n_samples=100, tow=1, C=1, depth=100, foo=0):
        for sample in range(n_samples):
            self.MCTS_rec(model, root, LTL_formula, trajectory.copy(), predicates,  N, W, Q, P, visited=visited, C=C, depth=depth-(len(trajectory)+1), foo=foo)
    ...
    def run_Q_test(self,policy, LTL_formula, predicates, start=None,T=100, runs=100, verbose=1):
