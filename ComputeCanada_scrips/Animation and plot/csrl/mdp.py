...
class GridMDP():
    ...
    def __init__(self, shape, structure=None, reward=None, label=None, A=Actions, p=0.8, figsize=6, lcmap={}, cmap=plt.cm.RdBu):
        ...
        self.enc = list(np.unique(self.label))
        self.enc.pop(self.enc.index(()))
    ...
    def build_grid_world(self, row, col):
        
        grid_world = np.zeros(self.shape)
        
        for i in range(self.shape[0]):
            for j in range(self.shape[1]):
                if self.structure[i,j]=='B': grid_world[i,j] = -1 # obsticale
                elif self.label[i,j]!=(): grid_world[i,j] = self.enc.index(self.label[i,j])+2 # Empty. look at lables
