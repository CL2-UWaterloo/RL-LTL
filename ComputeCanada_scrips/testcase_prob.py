...
def run_simulation(grid_world_shape, n_goals, formula, predicates, model, ordered_goals=False,
    ...
    for e in range(n_episodes):
        ...
        positions = [(np.where(state_history[i]==1)[0][0]*4 + np.where(state_history[i]==1)[1][0]) for i in range(len(state_history))]
        print("Pos:", positions)
        ...
        if training:
          ...
          model.fit(x_train, [y1_train, y2_train], epochs=50, verbose=0)
    ...
    return np.sum(rewards), positions[-1]    
...
print("Probabilistic Env, Training initiated...")
...
for i in range(n_games):
  res, loc = run_simulation(6, n_goals=1, formula=formula,predicates=predicates,model=model,
                            n_episodes=n_episodes, C=1, tow=0.1, n_steps=10, n_samples=400,random_move_chance=0.2,
...
for i in range(1):
  N, W, Q, P = {}, {}, {}, {}
  res, loc = run_simulation(6, n_goals=1, formula=formula,predicates=predicates,model=model,
                            n_episodes=1, C=1, tow=0.1, n_steps=20, n_samples=400,random_move_chance=0.2,
                            N=N, W=W, Q=Q, P=P, verbose=False, training=False)
